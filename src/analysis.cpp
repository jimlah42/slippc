...
std::string Analysis::asJson() {
  ...
  for (unsigned p = 0; p < 2; ++p) {
    ...
    for (unsigned d = Dynamic::__LAST - 1; d > 0; --d) {
      ss << JUIN(2, Dynamic::name[d], ap[p].dyn_counts[d])
         << ((d == 1) ? "\n" : ",\n");
    }
    ss << SPACE[ILEV] << "},\n";
    ...
    for (unsigned d = Dynamic::__LAST - 1; d > 0; --d) {
      ss << JFLT(2, Dynamic::name[d], ap[p].dyn_damage[d])
         << ((d == 1) ? "\n" : ",\n");
    }
    ss << SPACE[ILEV] << "},\n";
    ...
    for (unsigned i = 0; ap[p].attacks[i].frame > 0; ++i) {
      ...
      ss << JSTR(2, "cancel_name",
                 Cancel::shortname[ap[p].attacks[i].cancel_type])
         << ",\n";
    ...
    for (unsigned i = 0; ap[p].punishes[i].num_moves > 0; ++i) {
      ...
      ss << JSTR(2, "last_move_name",
                 Move::shortname[ap[p].punishes[i].last_move_id])
         << ",\n";
      ss << JSTR(2, "opening", "UNUSED") << ",\n";
      ss << JSTR(2, "kill_dir", Dir::name[ap[p].punishes[i].kill_dir]) << "\n";
      ss << SPACE[2 * ILEV] << "}"
         << ((ap[p].punishes[i + 1].num_moves > 0) ? ",\n" : "\n");
    }
    ss << SPACE[ILEV] << "]\n";
