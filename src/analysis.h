...
const unsigned MAX_ATTACKS = 65535;  // Maximum number of attacks per player per
                                     // game (increase later if needed)
const unsigned MAX_PUNISHES = 65535; // Maximum number of punishes per player
                                     // per game (increase later if needed)
...
struct Attack {
  ...
  uint8_t punish_id =
      0; // Index of the punish this move is a part of (one punish per opening)
  uint8_t hit_id = 0; // Index of the hit number for the current move (applies
                      // only to multihits)
  unsigned frame = 0; // Frame the move connected (0 == internal frame -123)
  float damage = 0;   // Damage dealt by the move
  uint8_t opening =
      0; // Opening type for first hit of punish (TODO: make an enum for this)
  uint8_t kill_dir =
      0; // Direction of kill, as specified in Dir enum (-1 = didn't kill)
...
struct Punish {
  uint8_t num_moves =
      0; // Number of moves (hitboxes connected) in the current punish
...
struct AnalysisPlayer {
  ...
  std::string tag_player =
      ""; // Tag of the player as recorded in the game metadata
  std::string tag_css =
      ""; // Tag of the player as chosen on the char. select screen
  ...
  unsigned pokes =
      0; // Number of times we poked (single hits not leading to punishes)
  ...
  float damage_dealt =
      0; // Total damage the player has dealth over the course of the game
  unsigned hits_blocked = 0;        // Number of hits blocked by shield
  unsigned shield_stabs = 0;        // Number of times we've shield stabbed our
                                    // opponent
  unsigned edge_cancel_aerials = 0; // Number of aerials edge cancelled
  unsigned edge_cancel_specials =
      0; // Number of special moves / air dodges / zairs edge cancelled
  ...
  unsigned reverse_edgeguards =
      0; // Number of times we scored a KO while recovering
  unsigned self_destructs = 0; // Number of times we died in neutral
  unsigned stage_spikes =
      0; // Number of times we KO'd our opponents with a stage spike
  ...
  float shield_damage =
      0; // Cumulative shield damage taken over the course of the game
  float shield_lowest =
      60; // Lowest shield health we had at any point in the match
  unsigned teeter_cancel_aerials = 0; // Number of aerials teeter cancelled
  unsigned teeter_cancel_specials =
      0; // Number of special moves / air dodges / zairs teeter cancelled
  unsigned total_openings = 0; // Total number of openings (pokes + neutral
                               // wins)
  float mean_kill_openings =
      0; // Average number of openings needed before getting a kill
  float mean_kill_percent = 0; // Average damage done before taking a stock
  float mean_opening_percent =
      0; // Average damage done during each opening / punish
  ...
  unsigned shieldstun_act_frames =
      0;                      // Total frames we spent acting out of shieldstun
  unsigned hitstun_times = 0; // Number of times we entered hitstun
  unsigned hitstun_act_frames = 0; // Total frames we spent acting out of
                                   // hitstun
  ...
  unsigned used_misc_moves =
      0; // Number of misc. moves (getups, ledge attacks, etc.) we threw out
  ...
  float actionability =
      0; // Mean actionability based on act out of stun and wait
  float neutral_wins_per_min =
      0; // Number of times we won neutral per minute spent in neutral
...
struct Analysis {
  ...
  unsigned parse_errors =
      0; // Number of errors that occurred during replay parsing
  ...
  std::string slippi_version =
      ""; // Version of Slippi the replay was recorded with
  std::string parser_version =
      ""; // Version of parser the replay was parsed with
  ...
  unsigned game_length =
      0;              // Length of the game in frames (0 == internal frame -123)
  ...
  Analysis(unsigned frame_count) {
    dynamics =
        new unsigned[frame_count]{0}; // List of dynamics active at each frame
